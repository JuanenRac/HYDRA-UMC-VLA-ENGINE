...
**現在実装済み：** アクショントークンのエンコード/デコードと軌道生成（`action_tokens.py`、`trajectory.py`）——上のフロー図の「アクショントークン」と「軌道生成器」のステップ——19 個のテストと実際の CLI 付き。

**まだ先で、実際のハードウェア/モデルの重みに阻まれている：** この v0 が既にデコードできるトークンを生成することになる、実際の VLA モデル推論（Hailo-10 向けに量子化された OpenVLA/RT-2）。
